...
optim_ignite_sgd <- torch::optimizer(
  ...
  initialize = function(params, lr, momentum = 0, dampening = 0, weight_decay = 0,
                        nesterov = FALSE) {
    self$opt <- rcpp_ignite_sgd(params, lr, momentum, dampening, weight_decay,
                              nesterov)
  },
  step = function() {
...
optim_ignite_adam <- torch::optimizer(
  ...
  initialize = function(params, lr, beta1 = 0.9, beta2 = 0.999, eps = 1e-8,
    ...
    self$opt <- rcpp_ignite_adam(params, lr, beta1, beta2, eps, weight_decay, amsgrad)
  },
  step = function() {
...
optim_ignite_adamw <- torch::optimizer(
  ...
  initialize = function(params, lr, beta1 = 0.9, beta2 = 0.999, eps = 1e-8,
    ...
    self$opt <- rcpp_ignite_adamw(params, lr, beta1, beta2, eps, weight_decay, amsgrad)
  },
  step = function() {
    optim_adamw_step(self$opt)
  },
  zero_grad = function() {
    optim_adamw_zero_grad(self$opt)
  }
)
...
optim_ignite_adagrad <- torch::optimizer(
  ...
  initialize = function(params, lr, lr_decay = 0, weight_decay = 0,
    ...
    self$opt <- rcpp_ignite_adagrad(params, lr, lr_decay, weight_decay,
                                   initial_accumulator_value, eps)
  },
  step = function() {
    optim_adagrad_step(self$opt)
  },
  zero_grad = function() {
    optim_adagrad_zero_grad(self$opt)
  }
)
...
optim_ignite_rmsprop <- torch::optimizer(
  ...
  initialize = function(params, lr, alpha = 0.99, eps = 1e-8, weight_decay = 0,
    ...
    self$opt <- rcpp_ignite_rmsprop(params, lr, alpha, eps, weight_decay,
                                   momentum, centered)
  },
  step = function() {
    optim_rmsprop_step(self$opt)
  },
  zero_grad = function() {
    optim_rmsprop_zero_grad(self$opt)
  }
)
