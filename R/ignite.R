ignite_function <- torch::autograd_function(
  forward = function(ctx, input, weights, bias, old_h, old_cell) {
    outputs <- rcpp_ignite_forward(input, weights, bias, old_h, old_cell)
...
nn_ignite <- torch::nn_module(
  ...
  forward = function(input, state) {
    ignite_function(input, self$weights, self$bias, state[[1]], state[[2]])
  }
)
